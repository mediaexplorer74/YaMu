...
namespace Yandex.Music.Api.API
{
    public class YUserAPI : YCommonAPI
    {
        ...
        {
            YAuth bbb = await new YAuthorizeRequest(api, storage)
        ...
        public void Authorize(AuthStorage storage, string login, string password)
        {
            ...
            {
                Debug.WriteLine("Authorize Exception: " + ex.Message);
            }
        }
        ...
        {
            YResponse<YAccountResult> yResponce = await new YGetAuthInfoRequest(api, storage)
                .Create()
