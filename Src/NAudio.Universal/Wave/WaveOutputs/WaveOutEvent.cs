...
namespace NAudio.Wave
{
    ...
    public class WaveOutEvent : IWavePlayer, IWavePosition
    {
        ...
        public void Init(IWaveProvider waveProvider)
        {
            ...
            int bufferSize = waveProvider.WaveFormat.ConvertLatencyToByteSize(
                (DesiredLatency + NumberOfBuffers - 1) / NumberOfBuffers);            

            MmResult result;
            lock (waveOutLock)
            {
                result = WaveInterop.waveOutOpenWindow(out hWaveOut, (IntPtr)DeviceNumber, 
                    waveStream.WaveFormat, 
                    default,//callbackEvent.SafeWaitHandle.DangerousGetHandle(), 
                    IntPtr.Zero,
        ...
        Task IWavePlayer.Init(IWaveProvider waveProvider)
        {
